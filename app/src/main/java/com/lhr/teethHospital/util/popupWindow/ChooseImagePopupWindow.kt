...
class ChooseImagePopupWindow(var context: Context, var fragment: OriginalImageFragment) : PopupWindow(), View.OnClickListener {
    var buttonTakePicture: Button
    var buttonSelectFromAlbum: Button
    ...
    init {
        ...
        buttonSelectFromAlbum = view.findViewById(R.id.buttonSelectFromAlbum)
        // 外部可點擊
        ...
        buttonSelectFromAlbum.setOnClickListener(this)
    }
...
//            activityForResult.launch(chooserIntent)
            ...
            fragment.selectFromAlbumResult.launch(chooserIntent)
        } else {
            //support for older than android 11
            Log.v("PPPPPPPPPPPP","support for older than android 11")
//            startActivityForResult(fragment.requireActivity(), chooserIntent, Model.IMAGE_REQUEST_CODE, null)
            fragment.selectFromAlbumResult.launch(chooserIntent)
        }
        dismiss()
    }
